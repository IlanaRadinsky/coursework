...
for (iter in 1:500) {
  ...
  #Create dataframe to store our observations
  sim_power <- NULL
  sim_power = matrix(0, obs_count, 2)
  sim_power[,1]<-obs_vec
  sim_power <- data.frame(sim_power)
  colnames(sim_power) <- c("sample_size","fifth_percentile")
  #Reset sample_means
  sample_means <- NULL
  sample_means = rep(0,total_samps)
  ...
  for (j in 1:obs_count) {
    ...
    sim_power[j,2] <- quantile(sample_means, 0.05)
  }
  
  temp <- sim_power %>%
    filter(fifth_percentile>=9) %>%
    filter(fifth_percentile==min(fifth_percentile)) 
